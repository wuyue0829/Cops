...
public class SkinParams {
    ...
    //设置默认推流码率，如果不设置，默认是800*1000；
    private int videoBitrate = 600*1000;
    //是否开启对焦功能，默认开启
    private boolean isOnTouch = true;
    //是否开启对焦点击动画，默认开启
    private boolean isOnAnimation = true;
    //对焦图片，在使用对焦功能时生效
    private int foceView = R.mipmap.letv_focus_auto;
...
//    private boolean isNowPush = false;
    ...
    //推流模式：允许单音频或单视频推流，只允许单音频推流，只允许单视频推流，不允许单音频或单视频推流
    private int publisherModel = RecorderContext.ALWAYS_PUBLISHER_MODE;
    //推流出错，自动重试功能。默认重试三次，每次间隔5秒
...
//    }
    ...
    public int getPublisherModel() {
        return publisherModel;
    }

    public void setPublisherModel(int publisherModel) {
