package com.pengdikj.cops.recorder;
...
public abstract class BaseSkinView extends FrameLayout {
    ...
    private void initIcon() {
        ...
        if (skinParams.isVolumeGain()) {
            volumeButton.setImageResource(R.mipmap.letv_recorder_voise_open);
            isVolume = true;
            setVolume(1);
        } else {
            volumeButton.setImageResource(R.mipmap.letv_recorder_voise_close);
            isVolume = false;
            setVolume(0);
        }
        if (skinParams.getCameraId() == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            ...
            isFlash = false;
            flashButton.setImageResource(R.mipmap.letv_recorder_flash_light_close);
        }
        ...
        public void onClick(View v) {
            ...
            if (i == R.id.imgB_back) {
                ...
                if (recording) {
                    stopPublisher();
                    hideTimerView();
                    openButton.setImageResource(R.mipmap.letv_recorder_open);
                } else {
                    time = 0;
                    openClickPublisher();
                }
                ...
                changeFlash(isFlash);
                if (isFlash) {
                    flashButton.setImageResource(R.mipmap.letv_recorder_flash_light_open);
                } else {
                    flashButton.setImageResource(R.mipmap.letv_recorder_flash_light_close);
                }
                v.setEnabled(true);
...
//        skinView.setBackgroundColor(0x01000001);
        ...
        if (skinParams.getSurfaceWidth() <= 0 || skinParams.getSurfaceHeight() <= 0) {
            ...
            params = new LayoutParams(skinParams.getSurfaceWidth(), skinParams.getSurfaceHeight(), Gravity.CENTER);
        }
...
//        publisher.setIRawFrameListener(rawFrameListener);
    ...
    private void showTimerView() {
        Log.i(TAG, "开始推流时间计数");
        openButton.setImageResource(R.mipmap.letv_recorder_stop);
        showTimer = true;
    ...
    private void enableFrontCameraMirror(boolean isMirror) {
        ...
        if (params.getCameraId() == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            ...
            if (isMirror) {
                mirrorButton.setImageResource(R.mipmap.letv_recorder_mirror_open);
            } else {
                mirrorButton.setImageResource(R.mipmap.letv_recorder_mirror_close);
            }
        } else {
        ...
        public void run() {
            ...
            if(skinView != null && !skinView.publisher.isRecording()){
                if(NetworkUtils.isNetAvailable(skinView.getContext())) {
                    ...
                    skinView.openButton.setImageResource(R.mipmap.letv_recorder_open);
